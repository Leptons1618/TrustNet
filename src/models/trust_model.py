    ...
    def predict_with_trust(self, inputs, return_features=False):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                # Use regular entropy as fallback
                results['odin_entropy'] = entropy_scores.cpu().numpy()
            
            # Mahalanobis distance if detector is available
            if self.mahalanobis_detector:
                try:
                    ...
                    results['mahalanobis_distance'] = np.ones(len(inputs)) * 10.0
